...
class Utilidades {
    ...
    public static function subirBoleta($param) {
        extract($param);
        mkdir(TMP_PATH_VENTA_ABSOLUTO . "$evento/" , 0777, TRUE);
        $carpetaDestino = TMP_PATH_BOLETAS_ABSOLUTO . "$evento/";
        //$archivoDestino = isset($_REQUEST["name"]) ? $_REQUEST["name"] : ('basura_' . substr(md5(rand()), 0, 10) . '.tmp');
        $archivoDestino = "boleta.jpg";
        ...
        if (is_file($archivoTemporal)) {
            if (is_dir($carpetaDestino)) {
                if (move_uploaded_file($archivoTemporal, $carpetaDestino . $archivoDestino)) {
                    ...
                    $sql = "UPDATE eventos SET diseno=? WHERE id_evento=?;";
                    $rs = $conexion->getPDO()->prepare($sql);
                    $url = TMP_PATH_BOLETAS_RELATIVO. "$evento/boleta.jpg";
                    if ($rs->execute(array($url, $evento))) {
                        echo("ok");
                    } else {
                        echo('{"jsonrpc" : "2.0", "error" : {"code": 102, "message": "Falló la actualizacion en la bd"}, "id" : "id"}');
                    }
                } else {
                    echo('{"jsonrpc" : "2.0", "error" : {"code": 102, "message": "Falló la carga de <b>' . $archivoDestino . '</b>."}, "id" : "id"}');
                }
            } else {
                $carpetaacrear = TMP_PATH_BOLETAS_ABSOLUTO . "$evento/";
                if (mkdir($carpetaacrear, 0777, TRUE)) {
                    if (move_uploaded_file($archivoTemporal, $carpetaDestino . $archivoDestino)) {
                        // Todo bien. Es posible que haya que hacer algo con el archivo que se acaba de subir
                        $sql = "UPDATE eventos SET diseno=? WHERE id_evento=?;";
                        $rs = $conexion->getPDO()->prepare($sql);
                    ...
                    echo('{"jsonrpc" : "2.0", "error" : {"code": 104, "message": "Falló la creacion de la carpeta ' . $carpetaacrear . '"}, "id" : "id"}');
                }
            }
        } else {
